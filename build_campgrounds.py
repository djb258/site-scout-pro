"""
Build Campgrounds Table
Searches Google Places API for campgrounds and RV parks in target counties.
Key demand driver for RV/boat storage - proximity to recreation.
"""
...
def build_campgrounds():
    conn = get_connection()
    cursor = conn.cursor(cursor_factory=RealDictCursor)

    print("=" * 70)
    print("BUILDING CAMPGROUNDS TABLE")
    print("=" * 70)
    ...
    print(f"\n   Searching {len(counties)} counties for campgrounds/RV parks...")

    search_terms = ["campground", "RV park", "camping"]
    seen_place_ids = set()
    total_inserted = 0
    ...
    for county in counties:
                ...
                try:
                    cursor.execute("""
                        INSERT INTO campgrounds (
                            name, campground_type, state, county, city, address,
        ...
        conn.commit()
        print(f"      Found {county_count} campgrounds/RV parks")

    print(f"\n" + "=" * 70)
    print(f"COMPLETE: {total_inserted} campgrounds/RV parks added")
    print("=" * 70)

    # Verify
    cursor.execute("""
        SELECT campground_type, COUNT(*) as cnt
        FROM campgrounds
        GROUP BY campground_type
    """)
    print("\n   By type:")
    for row in cursor.fetchall():
        print(f"      {row['campground_type']}: {row['cnt']}")
