...
export function FusionScoreCard({ fusion, compact = false }: FusionScoreCardProps) {
  ...
  const getScoreBg = (s: number) => {
    if (s >= 70) return "bg-green-500/20 border-green-500/30";
    if (s >= 40) return "bg-amber-500/20 border-amber-500/30";
    return "bg-red-500/20 border-red-500/30";
  };

  const getScoreLabel = (s: number) => {
    if (s >= 70) return "HIGH DEMAND";
      ...
      <div className={`rounded-lg border p-3 ${getScoreBg(score)}`}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-xs px-1.5 py-0.5 rounded ${getScoreBg(score)} ${getScoreColor(score)}`}
            >
              {getScoreLabel(score)}
            </span>
          </div>
    ...
    <div className={`rounded-xl border p-4 ${getScoreBg(score)}`}>
      ...
      <div className="grid grid-cols-3 gap-3 text-center">
        ...
        <div className="bg-card/50 rounded-lg p-2">
          ...
          <div className="text-xs text-muted-foreground">MF Units</div>
        </div>
        <div className="bg-card/50 rounded-lg p-2">
