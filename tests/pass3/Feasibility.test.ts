 ...
 * Doctrine ID: SS.03.07 (Pass 3 — Financial Modeling)
 *
 * Tests the core financial engine for Pass-3 Design/Calculator Hub.
 * Covers: Revenue, NOI, DSCR, Doctrine thresholds, Fatal flaws
 *
 * DOCTRINAL NOTE: Financial calculations belong in Pass 3.
 * Pass 2 is constraint-only eligibility checking.
 */
...
import {
  ...
  DEFAULT_OPEX_RATIO,
  FeasibilityInput,
} from '../../src/pass3/design_hub/spokes/Feasibility';
import type { PricingVerificationResult } from '../../src/pass2/underwriting_hub/types/pass2_types';
...
describe('Feasibility Spoke (Pass 3)', () => {
  describe('Spoke Metadata', () => {
    it('should return correct spoke ID (SS.03.07)', async () => {
      const input = createPassingDealInput();
      const result = await runFeasibility(input);
      expect(result.spokeId).toBe('SS.03.07');
    });
